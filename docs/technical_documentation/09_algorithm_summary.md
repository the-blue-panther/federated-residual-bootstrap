...
1. Sample $e^{\ast}$ with replacement from $\tilde{e}$
2. $y^{\ast} = \hat{y} + e^{\ast}$
3. $\hat{\beta}^{\ast} = (X^TX)^{-1}X^Ty^{\ast}$
4. Store $\hat{\beta}^{\ast}$

### 5.4 Output

$$\text{SE} = \text{sd}(\hat{\beta}^{\ast})$$
...
1. For each site $m$:
   - Sample $e^{\ast}_m$ from $\tilde{e}_m$
   - $y^{\ast}_m = \hat{y}_m + e^{\ast}_m$
2. **Run federated OLS on ALL sites** $\{X_m, y^{\ast}_m\}$
3. **Store ONE global** $\hat{\beta}^{\ast}$ **per iteration**
...
$$\text{SE}_{Local} = \text{sd}(\hat{\beta}^{\ast})$$

$$CI_{Local} = [Q_{\alpha/2}(\hat{\beta}^{\ast}), Q_{1-\alpha/2}(\hat{\beta}^{\ast})]$$
...
$$W(\hat{\beta}_{Local}^{\ast}, \hat{\beta}_{Central}^{\ast})$$
...
| $\hat{\beta}^{\ast}$ | Bootstrap coefficients |
| $X$ | Feature matrix |
| $y$ | Response vector |
| $e$ | Residuals |
